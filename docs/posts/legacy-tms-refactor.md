...
외부 운송관리시스템(TMS, Transport Management System)과 연동하는 코드가 있었다. 누가 만들었는지 아무도 몰랐다. 주석도 없고, 테스트도 없고, 변수명은 `a`, `b`, `temp`였다.

그리고 한 달에 3~5번은 반드시 실패했다.

실패하면 개발팀이 직접 TMS 관리자 페이지에 들어가서 수기로 처리했다. 매번 30분씩 걸렸다. 왜 실패했는지는 알 수 없었다. `catch`에서 `console.log(e)`만 찍고 끝이었으니까.
...
레거시 코드를 다룰 때 가장 두려운 건 "잘못 건드리면 더 망가질 것 같다"는 느낌이다. 이 코드는 어떻게 동작하는지 파악하기 어려웠다.

택한 방법은 **인터페이스를 먼저 정의하고 기존 동작을 인터페이스 뒤로 숨기는 것**이었다. 기존 코드를 건드리지 않고도 테스트할 수 있는 구조가 생긴다.
...
NestJS DI(의존성 주입)로 환경에 따라 어댑터를 교체한다.
...
TMS 서버가 일시적으로 내려가는 경우가 있었다. 이때 즉시 실패 처리하면 수기 대응이 필요하다. 지수 백오프(Exponential Backoff)로 자동 재시도한다.
...
|---|---|---|
| 수기 대응 | 월 3~5회 × 30분 | **0회** |
| 실패 감지 | 운영팀 연락 후 인지 | **Slack 즉시 알림** |
| 원인 추적 | 불가 | **Audit 로그 DB 조회** |
| 로컬 테스트 | 불가 (TMS 서버 필요) | **MockAdapter로 가능** |
| 재시도 | 없음 | **지수 백오프 3회** |

---

## 테스트 가능한 구조가 변경 가능한 구조다

레거시 코드를 다룰 때 가장 어려운 건 "이게 왜 이렇게 돼 있는지"를 모른다는 거다. 잘못 건드리면 더 망가질 것 같아서 손대기가 두렵다.

이 작업에서 택한 방법은 기존 코드를 한 번에 갈아엎는 게 아니라, 인터페이스를 먼저 정의하고 기존 동작을 인터페이스 뒤로 숨기는 것이었다. MockAdapter를 만들어서 로컬에서 테스트할 수 있게 된 순간부터, 코드를 자신 있게 고칠 수 있었다.

"테스트 가능한 구조"가 곧 "변경 가능한 구조"다.
