...
매월 1일, 15일, 말일. 이 세 날짜에는 개발팀과 운영팀이 함께 앉아서 결제를 처리했다. CMS가 API를 제공하지 않아서, 관리자 페이지에 직접 들어가 카드 정보를 입력하고 결제 버튼을 누르는 방식이었다.

한 번에 30분씩, 한 달에 세 번.

그리고 결제가 됐는지 안 됐는지 확인하려면 CMS 관리자 페이지를 다시 열어봐야 했다. 이력이 없었다. 고객이 "저 결제 됐나요?"라고 물어보면 CMS 화면을 직접 확인해야 했다.

---

## 처음부터 PG사가 바뀔 수 있다고 생각했다

설계를 시작할 때 "토스페이먼츠만 쓸 것"이 아니라 "나중에 PG사가 바뀔 수 있다"는 전제로 접근했다. 실제로 이니시스 도입이 예정돼 있었다.

PG사는 수수료, 정산 주기, 기능 지원 범위에 따라 바뀔 수 있다. 처음부터 토스페이먼츠에 직접 의존하는 코드를 짜면, PG사를 바꿀 때 결제 관련 코드 전체를 뜯어고쳐야 한다. 인터페이스 하나를 사이에 두면, 그 비용이 Adapter 하나 구현으로 줄어든다.
...
`rawResponse`에 PG사 원본 응답을 통째로 저장하는 이유가 있다. 나중에 "이 결제가 왜 실패했는지" 디버깅할 때, PG사 응답 원문이 있으면 고객센터에 문의하거나 원인을 파악하기 훨씬 쉽다. 스토리지 비용보다 디버깅 시간이 더 비싸다.
...
이 구조의 핵심 장점은 새 PG사를 추가할 때 `PaymentService`를 건드리지 않는다는 거다.

```
새 PG사 추가 시 작업:
  1. NewPgAdapter implements IPgAdapter 구현 (신규 파일)
  2. payment.module.ts에 provider 등록 (2줄 추가)
  3. selectAdapter()에 case 추가 (1줄 추가)

변경하지 않는 것:
  - PaymentService 비즈니스 로직
...
|---|---|---|
| 수기 작업 | 월 3회 × 30분 | **0회** |
| 결제 이력 | 스프레드시트 수기 기록 | **DB 자동 저장** |
| 실패 감지 | 다음 작업 때 인지 | **Slack 즉시 알림** |
| PG사 추가 | 전체 결제 로직 수정 | **Adapter 1개 추가** |
| 디버깅 | PG사 관리자 페이지 확인 | **DB + 원본 응답 조회** |

---

## "지금 당장 필요한가"보다 "나중에 없으면 얼마나 힘들어지는가"

Audit 구조를 설계할 때 "지금 당장 필요한가?"보다 "나중에 이게 없으면 얼마나 힘들어지는가"를 먼저 생각했다. 결제 이력이 없으면 고객 분쟁, 정산 검증, 장애 디버깅 모두 막막해진다.

PG 인터페이스도 마찬가지다. 이니시스 도입이 예정돼 있었지만, 없었더라도 같은 구조로 만들었을 것 같다. 결제 코드는 한 번 잘못 짜면 고치는 비용이 크다. 처음부터 교체 가능한 구조로 만드는 게 낫다.
